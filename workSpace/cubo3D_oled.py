# 3d square projection oled ssd1306

#Code from 'CHIPPERBITS' 3D wireframe cube with MicroPython
...
class Simulation:
    def __init__(self, width=128, height=64, fov=64, distance=4, rotateX=10, rotateY=7, rotateZ=1):
    ...
    def run(self):
        ...
        # Constante para animar escala do cubo
        grown = 1
        while 1:
            t = []
            ...
            '''
            if grown:
              self.projection[2] += 0.5
              if self.projection[2] == 101:
                grown = 0
            else:
              self.projection[2] -= 0.5
...
s = Simulation(fov = 44, distance = 2)
s.run()
